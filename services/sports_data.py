...
from core import Event, SportsProvider, Team, TeamStats
...
class SportsDataService:
    ...
    def get_head_coach(self, team_id: str, league: str) -> str:
        """Get head coach name for a team.

        This is ESPN-specific data - other providers may not support it.
        Uses long TTL since coaches rarely change mid-season.
        ...
        Returns:
            Coach name string, empty string if not available
        """
        cache_key = make_cache_key("coach", league, team_id)
        ...
        # Fetch from ESPN provider (only one that has coach data)
        for provider in self._providers:
            if provider.name == "espn" and provider.supports_league(league):
                # Access ESPN client's roster endpoint
                if hasattr(provider, "_client") and hasattr(provider._client, "get_team_roster"):
                    try:
                        roster_data = provider._client.get_team_roster(league, team_id)
                        if roster_data and "coach" in roster_data and roster_data["coach"]:
                            ...
                            # Cache with long TTL (24 hours - coaches don't change often)
                            self._cache.set(cache_key, coach_name, CACHE_TTL_TEAM_INFO)
                            return coach_name
        ...
        self._cache.set(cache_key, "", CACHE_TTL_TEAM_INFO)
        return ""
