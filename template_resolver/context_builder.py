...
Pure dataclass pipeline: Event → ContextBuilder → TemplateContext
"""

import logging

from core import Event, GameContext, TeamConfig, TeamStats, TemplateContext
from services.sports_data import SportsDataService
from utilities.sports import get_sport_from_league
...
class ContextBuilder:
    ...
    def build_for_event(
        ...
        Args:
            ...
            team_name: Override team name (from database)
            team_abbrev: Override team abbreviation
        ...
        team_config = TeamConfig(
            ...
            team_abbrev=team_abbrev or team.abbreviation,
    ...
    def build_minimal(
        ...
        Useful for filler content or when event data isn't available.
        """
        team_config = TeamConfig(
            team_id=team_id,
            league=league,
    ...
    def build_for_filler(
        ...
        """Build context for filler content (pregame/postgame/idle).

        Has no current game but may have next/last for suffix variables.
        """
        team_config = TeamConfig(
            team_id=team_id,
            league=league,
    ...
    def _build_game_context(
        ...
        # Fetch head coach
        head_coach = self._service.get_head_coach(team_id, league)
        ...
        return GameContext(
            ...
            # H2H and streaks could be added here with additional API calls
            h2h=None,
            streaks=None,
        )
...
def build_context_for_event(
    ...
    return builder.build_for_event(
        event, team_id, league, team_name=team_name, team_abbrev=team_abbrev
    )
