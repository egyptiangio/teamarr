...
# Timezone settings - used by utilities/tz.py
...
def get_user_timezone():
    ...
    Reads from database settings first, falls back to TZ env var, then default.
    This is used by utilities/tz.py for all timezone operations.
    """
...
def get_user_timezone_str() -> str:
    ...
    Reads from database settings first, falls back to TZ env var, then default.
    Wraps V1's utils/time_format.get_user_timezone() for consistency.
    """
    try:
        # Use V1's database-backed timezone function
        from utils.time_format import get_user_timezone as v1_get_tz
